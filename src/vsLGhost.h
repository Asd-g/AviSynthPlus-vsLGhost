...
class vsLGhost : public GenericVideoFilter
{
    ...
    std::array<aligned_unique_ptr<std::byte>, 3> buffer_plane;
    ...
    void (vsLGhost::*filter_ptr)(PVideoFrame& src, PVideoFrame& dst, IScriptEnvironment* env) noexcept;

public:
    vsLGhost(PClip _child, std::vector<int> mode, std::vector<int> shift, std::vector<int> intensity, int y, int u, int v, int opt, IScriptEnvironment* env);
